...
namespace FMI2 {
    ...
    // fmi2Status fmi2GetStringStatus(fmi2Component c, fmi2StatusKind s, fmi2String* value);
    delegate fmi2Status fmi2GetStringStatusDelegate(fmi2Component c, fmi2StatusKind s, out IntPtr value);
	...
	public class FMU : IDisposable
	{
        ...
        fmi2GetStringStatusDelegate fmi2GetStringStatus; // 멤버 변수로 선언
        ...
        public FMU(string fmuName, string instanceName,bool visible = false, bool loggingOn=false)
        {
            ...
            component = fmi2Instantiate(instanceName, (int)fmi2Type.fmi2CoSimulation, guid, resourceLocation, callbacks, visible ? fmi2True : fmi2False, loggingOn ? fmi2True : fmi2False);
        }
    ...
    p = dlsym(dll, fname);
            ...
            if (p == IntPtr.Zero)
            {
                Debug.LogError($"Failed to load function '{fname}' from DLL."); // 오류 로그
            ...
            {
                Debug.LogError($"Error creating delegate for '{fname}': {e.Message}");
                return null; // 또는 예외를 던짐
            }
        }
